...
const RightForm = ({ planId }) => {
  ...
  const handleSubmit = async () => {
    try {
      await axios.post('https://intime-backend.onrender.com/api/rightHandForm', {
        planId,
        type,
        brand,
        series,
      ...
      console.log('Right-hand side form submitted successfully');
      setIsModalOpen(true); // Open the modal when the submission is successful
    } catch (error) {
      console.error('Error submitting right-hand side form:', error);
    }
  };
  ...
  return (
    <div className="form-container">
      ...
      <div className="form-row">
        <div className="form-column">
          <label>
            ...
            <select value={series} onChange={(e) => setSeries(e.target.value)}>
              <option value="">Select Series</option>
              {Array.from({ length: 10 }, (_, i) => (i + 1).toString()).map((option) => (
      ...
      >
        <h2>Update Sent</h2>
        <p>The entries have been successfully updated.</p>
